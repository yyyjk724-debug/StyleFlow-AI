...
st.markdown(f'<p style="text-align: center; font-size: 1.1rem; opacity: 0.8; margin-bottom: 2rem;">{"어두운 밤에도 실시간으로 서식을 세탁하세요." if dark_mode else "AI의 서식을 지우고 당신의 스타일을 입히세요."}</p>', unsafe_allow_html=True)

# 4. 실시간 변환 메인 레이아웃 (좌우 배치)
...
with col_out:
    ...
    if st.session_state.input_text:
        ...
        if '|' in st.session_state.input_text:
            ...
            try:
                ...
                if len(valid_lines) > 1:
                    ...
                    st.text_area("Table Copy", value=tsv_data, height=120, label_visibility="collapsed")
                    st.caption("💡 위 박스의 내용을 복사해서 엑셀/한셀에 붙여넣으면 칸이 딱 맞습니다.")
        ...
        st.markdown(
            """
            <div style="padding: 150px 20px; text-align: center; border: 2px dashed #d1d5db; border-radius: 16px; color: #9ca3af; background-color: transparent; margin-top: 10px;">
                왼쪽창에 내용을 입력하면<br>결과가 실시간으로 나타납니다.
            </div>
            """, unsafe_allow_html=True
        )

st.markdown("<br><br><hr><p style='text-align: center; opacity: 0.5; font-size: 0.8rem;'>© 2026 StyleFlow AI. Built for Academic Excellence.</p>", unsafe_allow_html=True)
